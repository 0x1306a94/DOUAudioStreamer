...
- (void)analyseLevel:(float *)level result:(float *)result left:(BOOL)left {
    ...
    vDSP_vmul(channel, 1, window, 1, channel, 1, (vDSP_Length)(self.fftSize));
    ...
    for (int i = 0; i < count; i++) {
        float weighted = channelAmplitudes[i] * aWeights[i].floatValue;
        [weightedAmplitudes addObject: [NSNumber numberWithFloat:weighted]];
    }

    NSMutableArray<NSNumber *> *currentLevels = left ? self.spectrumBuffer.firstObject : self.spectrumBuffer.lastObject;
    NSMutableArray<NSNumber *> *spectrum = [NSMutableArray<NSNumber *> array];
    for (int t = 0; t < self.frequencyBands; t++) {
        float bandWidth = 44100.0 / (float)(self.fftSize * 1.0);
    ...
    for (int t = 0; t < self.frequencyBands; t++) {
        float oldVal = currentLevels[t].floatValue;
        float newVal = spectrum[t].floatValue;
        float resultVal = oldVal * self.spectrumSmooth + newVal * (1.0 - self.spectrumSmooth);
        currentLevels[t] = [NSNumber numberWithFloat:(isnan(resultVal) ? 0 : resultVal)];
        result[t] = resultVal;
    }
}
@end
